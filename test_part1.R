...
# maxIterations <- 5
alpha <- 0.85 #convergence rate, typical varies between 0.6 & 0.9

###### SET CONSTRAINTS
SP1day_ave <- apply(psc_burnout,2,mean)/30

llim_init <- SP1day_ave*7
ulim_init <- SP1day_ave*15
...
ulim_lower <- 0.9
llim_upper <- 1.1
ulim_upper <- SP1day_ave*25

###### INITIALISE DECISION VARIABLES. Stockpiles: Initial, UpperWarning & LowerWarning.
mus <- rep(NA,numVar)
sigmas <- rep(NA,numVar) 

### initial stock
dv_SPinitial <- SP1day_ave*10 
mus[1:psc_tot] <- dv_SPinitial #@ keep dv_SPinitial. I use it below in next 2 paragraphs
sigmas[1:psc_tot] <- mus[1:psc_tot] / 10

### upperWarning & lowerWarning
#lowerWarning
mus[psc_tot + 1:psc_tot] <- (llim_lower + dv_SPinitial*ulim_lower) /2
sigmas[psc_tot + 1:psc_tot] <- mus[psc_tot + 1:psc_tot] / 10

#upperWarning
mus[2*psc_tot + 1:psc_tot] <- (dv_SPinitial*llim_upper + ulim_upper) /2
sigmas[2*psc_tot + 1:psc_tot] <- mus[2*psc_tot + 1:psc_tot] / 10
